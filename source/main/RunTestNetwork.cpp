...
extern int RunTestNetwork(const Parameters& params, const ConfigSection& config, std::istream& firmware, NetworkInterface& network)
{
	...
	std::string service = config.Get("service", "");
	if (service.empty())
	{
		std::printf("ERROR: board '%s' configuration missing service.\n", params.board);
		return EXITCODE_INVALID_CONFIG;
	}

	// Connect to to the server.
	if (!network.Connect(host, service))
	{
		std::printf("ERROR: Unable to connect to host '%s' service '%s'\n", host.c_str(), service.c_str());
	...
	{
		std::printf("ERROR: Host '%s' service '%s' does not appear to be running HWTA protocol.\n", host.c_str(), service.c_str());
		return EXITCODE_NETWORK_ERROR;
	}
	...
	{
		std::printf("ERROR: Host '%s' service '%s' is running version %x, but we only support down to %x.\n", host.c_str(), service.c_str(), remote_version, HWTA_VERSION_MIN_SUPPORTED);
		return EXITCODE_NETWORK_ERROR;
	}
	...
	if (!connection)
	{
		std::printf("ERROR: Host '%s' service '%s' dropped connection.\n", host.c_str(), service.c_str());
		return EXITCODE_NETWORK_ERROR;
	}
	if (status != HWTA_RESPONSE_OK)
	{
		std::printf("ERROR: Host '%s' service '%s' refused to run tests on board '%s' with error %s (%d).\n", host.c_str(), service.c_str(), params.board, ErrorString(status), status);
		return EXITCODE_NETWORK_ERROR;
	}
	...
	if (!connection)
	{
		std::printf("ERROR: Host '%s' service '%s' dropped connection during tests.\n", host.c_str(), service.c_str());
		return EXITCODE_NETWORK_ERROR;
	}
	...
	{
		std::printf("Tests passed on host '%s' service '%s'\n", host.c_str(), service.c_str());
		return EXITCODE_SUCCESS;
	}

	if (test_status == HWTA_RESPONSE_TESTS_FAIL)
	{
		std::printf("Tests failed on host '%s' service '%s'\n", host.c_str(), service.c_str());
		return EXITCODE_TESTS_FAILED;
	}

	if (test_status == HWTA_RESPONSE_TESTS_TIMEOUT)
	{
		std::printf("Tests timed out on host '%s' service '%s'\n", host.c_str(), service.c_str());
		return EXITCODE_TESTS_TIMEOUT;
	}

	std::printf("ERROR: Host '%s' service '%s' returned unexpected test status '%s' (%d).\n", host.c_str(), service.c_str(), ErrorString(test_status), test_status);
	return EXITCODE_NETWORK_ERROR;
}
